            ...
            <div class="flex items-center gap-3">
                <form method="GET" action="{{ route('users.index') }}" class="relative group" data-users-search>
                    ...
                    @if(!empty($q ?? request('q')))
                        <a href="{{ route('users.index') }}" class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-700" title="Xóa tìm kiếm" aria-label="Clear search">
                ...
                @if ($isAdmin)
                    <a href="{{ route('users.create') }}"
                       class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-2xl shadow-lg shadow-indigo-200 transition-all active:scale-95">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="relative">
                                     <img class="w-10 h-10 rounded-2xl border-2 border-white shadow-md group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 object-cover" 
                                        src="{{ $user->avatar ? asset('storage/' . $user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&bg=6366f1&color=fff' }}" 
                                        alt="Avatar">
                                    <span class="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-500 border-2 border-white rounded-full"></span>
                                </div>
                                <div>
                                    <p class="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors tracking-tight">{{ $user->name }}</p>
                        ...
                        <td class="px-6 py-5">
                            ...
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border
                                {{ $role === 'administrator' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100' }}">
                ...
                <div class="w-full max-w-sm rounded-3xl bg-white border border-slate-200 shadow-2xl overflow-hidden">
                    <div class="px-5 py-4">
                        ...
                        <p class="mt-1 text-xs font-medium text-slate-500">
                            Bạn sắp xóa <span id="users-delete-name" class="font-black text-slate-700"></span>. Hành động này không thể hoàn tác.
                        </p>
                    </div>
                    <div class="px-5 pb-5 flex items-center justify-end gap-2">
                        <button id="users-delete-cancel" type="button"
                            class="px-4 py-2 rounded-2xl text-sm font-bold text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-all">
                            Hủy
                        </button>
                        <button id="users-delete-confirm" type="button"
                    ...
                    <form id="users-delete-form" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
